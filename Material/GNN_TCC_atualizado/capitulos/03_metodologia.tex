...
\path{00_construir_grafos_fakenewsnet.py}. O \emph{script} efetua o
...
O \emph{script} \path{gerar_folds.py} instancia
\path{StratifiedKFold(n_splits=10, shuffle=True, random_state=42)} do
\emph{scikit-learn} sobre os 754 grafos do FakeNewsNet e persiste os
\'indices em \path{data/folds_fnn.pt}. A escolha de $k=10$ segue
...
(\path{scipy.stats.ttest_rel}) utilizado para comparar dois modelos
...
\S\ref{sec:cross_dataset} sobre o UPFD, utiliza-se a variante
...
modelo representa o \emph{confound trivial} da detec\c{c}\~ao de fake news em
...
reporta acur\'acia; usa-se F1-macro para alinhar com pr\'aticas mais
...
Para comparar dois modelos sob o mesmo \emph{dataset}, utiliza-se o
\emph{teste t pareado} (\path{scipy.stats.ttest_rel}) sobre os
...
real (\S\ref{sec:porque}), reporta-se o \emph{tamanho de efeito de Cohen}
...
adotam-se tr\^es t\'ecnicas complementares aplicadas sobre o
...
Implementa-se via \emph{rolling mean} de janela proporcional ($10\%$
...
regime documentado por Barab\'asi e Albert~\cite{barabasi1999emergence}
e Clauset et al.~\cite{clauset2009powerlaw} como caracter\'istico de redes sociais. Nesse cen\'ario, a
...
\path{torch_geometric.explain.Explainer} com algoritmo
\path{GNNExplainer(epochs=200)} e \path{edge_mask_type="object"}, em
...
Ap\^endice~\ref{ap:aplicacao}, combinam-se o classificador textual
...
\path{torch.manual_seed}, \path{numpy.random.seed},
\path{random.seed}, \path{StratifiedKFold(random_state=42)} e os
construtores dos classificadores do \emph{scikit-learn} (\path{n_jobs=-1}
\'e adotado para o RF, sem efeito sobre determinismo). Para os
experimentos sens\'iveis \`a inicializa\c{c}\~ao, conduz-se varredura
expl\'icita sobre 10 \emph{seeds} independentes em \S\ref{sec:topo_sem_texto}
e reportam-se m\'edia e desvio-padr\~ao.
